...
class AppViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val selectedTrip = MutableLiveData<NetworkResults<InProgeassModel>>()
    ...
    fun retriveSelectedTrip(
        orderId: String
    ) {
        viewModelScope.launch {
            selectedTrip.value = repo.tripById(
                orderId,
                language
            )
        }
...
//    tripTrakingLive
    ...
    fun retrieveProfileById(
        ...
        viewModelScope.launch {
            getProfileLiveData.value = repo.getProfileById(
                uid
            )
        }
    }
